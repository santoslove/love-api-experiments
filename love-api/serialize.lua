return function(api)
    ...
    local function DESCRIPTION(file, item)
        ...
        if item.descriptiont then
            KEYOPEN(file, 'descriptiont')
            local codes = {}
            for languageCode, _ in pairs(item.descriptiont or {}) do
                table.insert(codes, languageCode)
            end
            table.sort(codes)
            for _, languageCode in ipairs(codes) do
                K(file, item.descriptiont, languageCode)
            end
            CLOSE(file)
        end
    end

    local function MINIDESCRIPTION(file, item)
        K(file, item, 'minidescription')
        KEYOPEN(file, 'minidescriptiont')
        local codes = {}
        for languageCode, _ in pairs(item.minidescriptiont) do
            table.insert(codes, languageCode)
        end
        table.sort(codes)
        for _, languageCode in ipairs(codes) do
            K(file, item.minidescriptiont, languageCode)
        end
        CLOSE(file)
    end
        ...
        if #functions > 0 then
            for _, function_ in ipairs(functions or {}) do
                if not isLoveModule or function_.module.name == 'love' then
                    ...
                    MINIDESCRIPTION(file, function_)
